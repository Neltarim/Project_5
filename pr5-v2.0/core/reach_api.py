    ...
    def dump_cat(self, cat):
        ...
        for prod in prods:
            try: #in order to reduce the amount of useless data in the products, we only save
                 # those attributes
                simplified = {
                'generic_name'          : prod['generic_name'],         #name
                'labels_hierarchy'      : prod['labels_hierarchy'],     #sanitary labels
                'image_url'             : prod['image_url'],            #Url for the icon image
                'categories_hierarchy'  : prod['categories_hierarchy'], #sub categories
                'categories'            : prod['categories'],           #also sub categories (just in case)
                'nutrient_levels'       : prod['nutrient_levels']       #nutrient levels
                }

                if simplified['generic_name'] != '': #avoid the products without name
